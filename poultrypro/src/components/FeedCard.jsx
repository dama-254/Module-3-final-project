export default function FeedCard({ record, onEdit, onDelete }) {
  return (
    <div style={{ background: '#0d1f0d', border: '1px solid rgba(134,239,172,0.09)', borderRadius: '12px', padding: '14px', marginBottom: '8px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
      <div>
        <p style={{ fontSize: '13px', fontWeight: 600, color: '#e8f5e9', marginBottom: '3px' }}>{record.feedType}</p>
        <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)' }}>{record.batchId} · {record.date}</p>
      </div>
      <div style={{ textAlign: 'right' }}>
        <p style={{ fontSize: '13px', fontWeight: 600, color: '#93c5fd', marginBottom: '6px' }}>{record.quantityKg} kg · KSh {Number(record.costKsh).toLocaleString()}</p>
